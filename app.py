...
if not GOOGLE_VISION_API_KEY:
    st.error("🛑 Définis `GOOGLE_VISION_API_KEY` dans les Secrets Streamlit Cloud.")
    st.stop()
VISION_URL = f"https://vision.googleapis.com/v1/images:annotate?key={GOOGLE_VISION_API_KEY}"

# --- OCR Google Vision pour image bytes ---
...
# --- Lecture texte PDF natif (si possible) ---
...
# --- Parsing multi-variantes puis fallback générique ---
...
def parse_with_fallback(raw: str) -> pd.DataFrame:
    df = parse_robust(raw)
    if df.empty:
        st.warning("⚠️ Pas de mots-clés trouvés, fallback générique.")
        df = parse_generic(raw)
        if df.empty:
            st.warning("⚠️ Rien non plus en parsing générique.")
    return df
...
def read_excel_bytes(x: bytes) -> pd.DataFrame:
    try:
        ...
        st.error("❌ Erreur lecture Excel: "+str(e))
